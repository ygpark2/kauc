...
{
	private $table_name			= 'geo_logs';
	private $contents_table_name	= 'geo_log_contents';
	private $tag_join_table_name	= 'geo_log_tag';
	...
	{
		$query = $this->db->get_where($this->table_name, array('id' => $id, 'user_id' => $user_id));
		if ($query->num_rows() == 1) {
			return true;
		} else {
			return false;
	...
	function retrieve($id)
	{
		...
		if ($log_query->num_rows() == 1) {
			$geoLog = array_pop($log_query->result());
			...
			$this->db->where($this->tag_join_table_name.'.geo_log_id', $id);
			...
			if ($tag_query->num_rows() > 1) {
				...
				$geoLog->tag = $comma_separated_tag;
			} else {
				if ( is_array($result_rows) && count($result_rows) ==  1 ) {
					$geoLog->tag = array_pop($result_rows)->tag;
				} else {
					$geoLog->tag = '';
				}
			}
	...
	function create($geo_loc, $head, $content)
	{
		...
		if ( $this->db->query($insert_string) ) {
			$content['geo_log_id'] = $this->db->insert_id();
			if ( $this->db->insert($this->contents_table_name, $content) ) {
				return $content['geo_log_id'];
			}
		}
		return NULL;
	}
	...
	function update($id, $user_id, $head, $content)
	{
		if ( $this->db->update($this->table_name, $head, array('id' => $id, 'user_id' => $user_id)) ) {
			if ( $this->db->update($this->contents_table_name, $content, array('geo_log_id' => $id)) ) {
				return true;
			}
		}
		return NULL;
	 ...
	 * @return	array
	 */
	function delete($log_id, $user_id)
	{
		if ( $this->db->delete($this->table_name, array('id' => $log_id, 'user_id' => $user_id)) ) {
			if ( $this->db->delete($this->contents_table_name, array('geo_log_id' => $log_id)) ) {
				return true;
			}
		}
		return NULL;
