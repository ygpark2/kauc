...
{
	private $table_name			= 'geo_businesses';
	private $contents_table_name	= 'geo_business_contents';
	...
	function create($geo_loc, $head, $content)
	{
		...
		if ( $this->db->query($insert_string) ) {
			$content['geo_business_id'] = $this->db->insert_id();
			if ( $this->db->insert($this->contents_table_name, $content) ) {
				return $content['geo_business_id'];
			}
		}
		return NULL;
	}
	...
	function update($id, $user_id, $head, $content)
	{
		if ( $this->db->update($this->table_name, $head, array('id' => $id, 'user_id' => $user_id)) ) {
			if ( $this->db->update($this->contents_table_name, $content, array('geo_business_id' => $id)) ) {
				return true;
			}
		}
		return NULL;
	 ...
	 * @return	array
	 */
	function delete($business_id, $user_id)
	{
		if ( $this->db->delete($this->table_name, array('id' => $business_id, 'user_id' => $user_id)) ) {
			if ( $this->db->delete($this->contents_table_name, array('geo_business_id' => $business_id)) ) {
				return true;
			}
		}
		return NULL;
